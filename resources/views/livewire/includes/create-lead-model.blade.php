<div x-data="{ showModal: @entangle('isOpen'),
     ...
     x-show="showModal"
     x-init="
                // Watch currentTab for changes (optional, for logging or debugging)
                $watch('currentTab', value => {
                    console.log('Current tab:', value);
                });

                // Watch completedTabs for changes (optional, for logging or debugging)
                $watch('completedTabs', value => {
                    console.log('Completed tabs updated:', value);
                });

                // Listen for the 'tabChanged' event from Livewire to update currentTab
                Livewire.on('tabChanged', (tab, completedTabs) => {
                    currentTab = tab;
                    completedTabs = completedTabs; // Update completed tabs in Alpine.js
                });
            "
     class="fixed z-10 inset-0 overflow-y-auto" style="display: block; background-color: rgba(179, 179, 179, 0.23); z-index: 50;">
    <div class="flex items-center justify-center min-h-screen">
        <div class="bg-white shadow-md rounded-3xl px-8 pt-7 pb-8 mb-4 w-3/4  relative" style="overflow: auto;height: 690px;">


                    <div class="bg-white overflow-hidden shadow-sm  ">
                        <!-- Close button -->
                        <button @click="showModal = false; @this.closeModal()" class="transition absolute top-1 border rounded right-2 text-teal-600 hover:text-white hover:border-teal-500 hover:bg-teal-500 ">
                        ...
                        <div class="container mx-auto p-4 border sm:rounded-lg border-r-indigo-50">
                            ...
                            <div class="border p-6 rounded">
                                <h2 class="text-2xl font-semibold mb-7 text-center">Create New Lead</h2>
                                @if(session('message'))
                                ...
                                <form wire:submit.prevent="store" class="w-9/12 mx-auto">
